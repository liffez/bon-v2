...
if (Number.isInteger(FIRMA) && !FAMILIE) {
    FAMILIE = db.prepare('SELECT name FROM companies WHERE id = ?').get(FIRMA)?.name || '';
}
if (!FAMILIE) { console.error('Brug: --familie "<navn>" eller --company <id>'); process.exit(2); }
...
const fjern = new Set([...ord, initialer]);
...
for (const c of iFamilien.slice().sort((a, b) => b.bons - a.bons)) {
    ...
    const stamme = (a, b) => a.split(' ').filter(x => x.length >= 3 && !STRUKTUR.has(x))
        .some(x => b.split(' ').filter(y => y.length >= 3 && !STRUKTUR.has(y))
            .some(y => y.startsWith(x) || x.startsWith(y)));
...
console.log(`\nFamilie "${FAMILIE}": ${iFamilien.length} firma-rækker i Bon`);
...
console.log('Klyngerne er bevidst forsigtige: to grupper der burde være én (fx "math" og');
console.log('"institut for matematiske fag") er lettere at rette i hånden end én sammenlægning');
console.log('af to afdelinger der ikke hørte sammen. Læs dem igennem før du lægger sammen.\n');
